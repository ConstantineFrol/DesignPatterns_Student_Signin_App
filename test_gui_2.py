...
def Pains():
    ...
    def addPain():
        ...
        # ----- Calculer Total ---
        Tot2 = (p5_5) + (p6_6) + (p7_7) + (p8_8) + (p9_9) + (p10_10) + (p11_11) + (p12_12) + (p13_13) + (p14_14) + (
            p15_15)
        Total_Gen += Tot2
    ...
    lbl_sandwich_vide = Label(right_frame, text="Sandwich Vide", font=("courier", 14, "bold"),
                              background=backgroundcolor)
    lbl_sandwich_vide.grid(row=0, column=0, sticky="w")

    entry_sandwich_vide = Spinbox(right_frame, background=backgroundcolor, from_=0, to=50, width=6,
                                  textvariable=E_sandwich_vide)
    entry_sandwich_vide.grid(row=0, column=1, pady=4)
    # -------------
    lbl_sandwich_vegetarien = Label(right_frame, text="Sandwich Végetarian", font=("courier", 14, "bold"),
                                    background=backgroundcolor)
    lbl_sandwich_vegetarien.grid(row=1, column=0, sticky="w")

    entry_sandwich_vegetarien = Spinbox(right_frame, background=backgroundcolor, from_=0, to=50, width=6,
                                        textvariable=E_sandwich_vegetarien)
    entry_sandwich_vegetarien.grid(row=1, column=1, pady=4)
    # -------------
    lbl_sandwich_jambon_ou_fromage = Label(right_frame, text="Sandwich\nJambonOuFromage", font=("courier", 14, "bold"),
                                           background=backgroundcolor)
    lbl_sandwich_jambon_ou_fromage.grid(row=2, column=0, sticky="w")

    entry_sandwich_jambon_ou_fromage = Spinbox(right_frame, background=backgroundcolor, from_=0, to=50, width=6,
                                               textvariable=E_sandwich_jambon_ou_fromage)
    entry_sandwich_jambon_ou_fromage.grid(row=2, column=1)
    # -------------
    lbl_sandwich_jambon_et_Fromage = Label(right_frame, text="Sandwich\nJambon+Fromage", font=("courier", 14, "bold"),
                                           background=backgroundcolor)
    lbl_sandwich_jambon_et_Fromage.grid(row=3, column=0, sticky="w")

    entry_sandwich_jambon_et_Fromage = Spinbox(right_frame, background=backgroundcolor, from_=0, to=50, width=6,
                                               textvariable=E_sandwich_jambon_et_Fromage)
    entry_sandwich_jambon_et_Fromage.grid(row=3, column=1)
    # -------------
    lbl_sandwich_poulet = Label(right_frame, text="Sandwich au poulet", font=("courier", 14, "bold"),
                                background=backgroundcolor)
    lbl_sandwich_poulet.grid(row=4, column=0, sticky="w")

    entry_sandwich_poulet = Spinbox(right_frame, background=backgroundcolor, from_=0, to=50, width=6,
                                    textvariable=E_sandwich_poulet)
    entry_sandwich_poulet.grid(row=4, column=1, pady=3)
    # -------------
    lbl_sandwich_thon = Label(right_frame, text="Sandwich au thon", font=("courier", 14, "bold"),
                              background=backgroundcolor)
    lbl_sandwich_thon.grid(row=5, column=0, sticky="w")

    entry_sandwich_thon = Spinbox(right_frame, background=backgroundcolor, from_=0, to=50, width=6,
                                  textvariable=E_sandwich_thon)
    entry_sandwich_thon.grid(row=5, column=1, pady=3)
    # -------------
    lbl_sandwich_club = Label(right_frame, text="Sandwich club", font=("courier", 14, "bold"),
                              background=backgroundcolor)
    lbl_sandwich_club.grid(row=6, column=0, sticky="w")

    entry_sandwich_club = Spinbox(right_frame, background=backgroundcolor, from_=0, to=50, width=6,
                                  textvariable=E_sandwich_club)
    ...
    entry_hamburger = Spinbox(right_frame, background=backgroundcolor, from_=0, to=50, width=6,
    ...
    entry_cheeseburger = Spinbox(right_frame, background=backgroundcolor, from_=0, to=50, width=6,
                                 textvariable=E_cheeseburger)
    entry_cheeseburger.grid(row=8, column=1, pady=3)
    # -------------
    lbl_croque_monsieur = Label(right_frame, text="Croque Monsieur", font=("courier", 14, "bold"),
                                background=backgroundcolor)
    lbl_croque_monsieur.grid(row=9, column=0, sticky="w")

    entry_croque_monsieur = Spinbox(right_frame, background=backgroundcolor, from_=0, to=50, width=6,
                                    textvariable=E_croque_monsieur)
    entry_croque_monsieur.grid(row=9, column=1, pady=3)
    # -------------
    lbl_croque_madame = Label(right_frame, text="Croque Madame", font=("courier", 14, "bold"),
                              background=backgroundcolor)
    lbl_croque_madame.grid(row=10, column=0, sticky="w")

    entry_croque_madame = Spinbox(right_frame, background=backgroundcolor, from_=0, to=50, width=6,
                                  textvariable=E_croque_madame)
...
def Soupes():
    ...
    # ======= 1 ========
    lbl_SoupecremeLegu = Label(right_frame, text="Soupe Creme\nLegumes", font=("courier", 14, "bold"),
                               background=backgroundcolor)
    lbl_SoupecremeLegu.grid(row=0, column=0)
    # ------
    entry_SoupecremeLegu = Spinbox(right_frame, background=backgroundcolor, from_=0, to=50, width=6,
                                   textvariable=E_SoupecremeLegu)
    entry_SoupecremeLegu.grid(row=0, column=1)
    # =========2=========
    lbl_SoupecremePoisson = Label(right_frame, text="Soupe creme\nde poisson", font=("courier", 14, "bold"),
                                  background=backgroundcolor)
    lbl_SoupecremePoisson.grid(row=1, column=0)
    # ==================
    entry_SoupecremePoisson = Spinbox(right_frame, background=backgroundcolor, from_=0, to=50, width=6,
                                      textvariable=E_SoupecremePoisson)
    entry_SoupecremePoisson.grid(row=1, column=1)
    # =========3=========
    lbl_SoupecremePoulet = Label(right_frame, text="Soupe creme\nde poulet", font=("courier", 14, "bold"),
                                 background=backgroundcolor)
    lbl_SoupecremePoulet.grid(row=2, column=0)
    # ==================
    entry_SoupecremePoulet = Spinbox(right_frame, background=backgroundcolor, from_=0, to=50, width=6,
                                     textvariable=E_SoupecremePoulet)
    ...
    # ==================
    entry_SoupecremePoireaux = Spinbox(right_frame, background=backgroundcolor, from_=0, to=50, width=6,
                                       textvariable=E_SoupecremePoireaux)
